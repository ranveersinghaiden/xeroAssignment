 ...
 *
 * Products   : myeroad */
public class Empty_state_renders_without_error_when_there_are_no_logon_recordsUiTest {
    ...
    @Test
    @DisplayName("Empty state renders without error when there are no logon records")
    void test_empty_state_renders_without_error_when_there_are_no_logon_records() {
        // GIVEN
        // the API returns an empty array of driver logon records

        // WHEN
        driver.get("http://localhost:3000");
        // the driver logon card table is rendered

        // THEN
        assertThat(driver.getTitle()).isNotEmpty();
        // the empty state UI is displayed and no JavaScript error is thrown
    }
}
